            ...
            for(size_t i=0; i < size; i++){
                ...
                for (size_t l=1; l < npts; ++l)
                    for(size_t j=0; j<size; j++){
                        ...
                        // gsl_matrix_set(F,size*(k-1)+i-1,size*(l-1)+j-1,dasi*interpolation::beta(nodei,d.nodes(j,points[l-1],points[l]))*d.quadratureWeights(j,points[l-1],points[l]))*pi[size*(l-1)+j-1];
                    }
                for(size_t j=0; j<size; j++){
                    if(k==1 && j==0)
                        ++j;
